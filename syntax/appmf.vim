" Vim syntax file
" Language:         AppMF (deltaOS Application Manifest) 1.2
...
syn match appMFenumDirective '%+\s\+' contained nextgroup=yamlTagHandle
...
syn match appMFVersionDirective '%AppMF\s\+'  contained nextgroup=yamlYAMLVersion
...
execute 'syn match yamlReservedDirective contained nextgroup=yamlComment '.
            \string('%\%(\%(+\|AppMF\)\s\)\@!'.s:ns_directive_name)
